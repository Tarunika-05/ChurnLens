...
def build_tenure_cohorts(df: pd.DataFrame) -> pd.DataFrame:
    ...
    cohort_data = (
        df_cohort.groupby(["tenure_bucket", "InternetService"])
        .agg(total_customers=("customerID", "count"), retained_customers=("retained", "sum"))
        .reset_index()
    )

    cohort_data["retention_rate"] = (
        cohort_data["retained_customers"] / cohort_data["total_customers"]
    )
    return cohort_data
...
def build_retention_heatmap_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    pivot = cohort_data.pivot(
        index="InternetService", columns="tenure_bucket", values="retention_rate"
    )
...
def compute_cohort_metrics(df: pd.DataFrame) -> pd.DataFrame:
    ...
    metrics = (
        df_cohort.groupby("tenure_bucket")
        .agg(
