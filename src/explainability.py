...
def explain_prediction(
    ...
    try:
        if preprocessor is not None:
            X_transformed = preprocessor.transform(instance)
        else:
            X_transformed = instance
...
def global_shap_summary(explainer, preprocessor, X: pd.DataFrame, feature_names: list[str], save_path: str = None):
