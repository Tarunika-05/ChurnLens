...
async def predict_single(
    request: PredictionRequest, model=Depends(get_model), explain: bool = False
) -> PredictionResponse:
    ...
    try:
        ...
        if missing:
            raise HTTPException(
                status_code=422, detail=f"Missing required features after engineering: {missing}"
            )
...
async def predict_batch(
    file: UploadFile = File(...), model=Depends(get_model)
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error processing batch: {e}") from e
